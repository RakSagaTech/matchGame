...
class MatchGame extends Component {
  ...
  thumbnail = id => {
    const {imageId, seconds, isGameOver} = this.state
    const isImageMatched = imageId === id
    if (seconds <= 0 || isImageMatched === false) {
      this.clearTimer()
      this.setState({isGameOver: !isGameOver, seconds: 0})
    } else {
  ...
  renderGameView = () => {
    ...
    return (
      <div className="game-view-container">
        <img src={image} className="image" alt="match" />
        ...
        <ul className="thumbnail-list">
          {filterImages.map(eachThumb => (
            <ThumbnailItem
              key={eachThumb.id}
              thumbnailDetails={eachThumb}
  ...
  render() {
    ...
    return (
      <div className="app-container">
        ...
        {isGameOver ? this.renderGamerOverView() : this.renderGameView()}
      </div>
    )
  }
}
